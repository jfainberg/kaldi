...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "<pdf-post-rspecifier> <aux-task-rspecifier> <egs-out>\n"
        "\n"
    ...
    std::string feature_rspecifier = po.GetArg(1),
        ...
        aux_task_rspecifier = po.GetArg(3),
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!pdf_post_reader.HasKey(key) || !aux_task_reader.HasKey(key)) {
        num_err++;
        if (!aux_task_reader.HasKey(key)) {
          KALDI_WARN << "No aux posterior for key " << key;
        } else {
          KALDI_WARN << "No pdf-level posterior posterior for key " << key;
        }
      } else {
        ...
        ProcessFile(feats, ivector_feats, pdf_post, aux_post, key, compress,
