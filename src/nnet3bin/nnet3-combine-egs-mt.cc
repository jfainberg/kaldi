...
//
// The idea of this script is to be used to combine two multi-task egs archives,
// when doing unsupervised adaptation on a test set. 
// E.g. we would combine training egs with labels for two output tasks
// and test egs with labels only for the auxiliary output task.
// This way we can interleave training at the egs level.
...
void FilterExample(const NnetExample &eg,
  ...
  for (size_t i = 0; i < eg.io.size(); i++) {
    ...
    } else if (name == "output" || name == "output_aux") {
      min_t = min_output_t;
      max_t = max_output_t;
      is_input_or_output = true;
    } else {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Combine and copy examples (single frames or fixed-size groups of frames) for neural\n"
        ...
        "\n"
        "Usage:  nnet3-combine-egs-mt [options] <egs-rspecifier> <egs-rspecifier2> <egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
    ...
    std::string test_examples_rspecifier = po.GetArg(2);

    SequentialNnetExampleReader example_reader(examples_rspecifier);
    SequentialNnetExampleReader example_reader_test(test_examples_rspecifier);
    ...
    for (; !example_reader_test.Done(); example_reader_test.Next(), num_read++) {
      // count is normally 1; could be 0, or possibly >1.
      int32 count = GetCount(keep_proportion);  
      std::string key = example_reader_test.Key();
      const NnetExample &eg = example_reader_test.Value();
      for (int32 c = 0; c < count; c++) {
        int32 index = (random ? Rand() : num_written) % num_outputs;
        if (frame_str == "" && left_context == -1 && right_context == -1) {
          example_writers[index]->Write(key, eg);
