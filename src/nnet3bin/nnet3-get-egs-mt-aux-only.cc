...
// WIP: Multi-task version, 2015 Joachim Fainberg
...
static void ProcessFile(const MatrixBase<BaseFloat> &feats,
  ...
  KALDI_ASSERT(feats.NumRows() == static_cast<int32>(aux_post.size()));
  ...
  for (int32 t = 0; t < feats.NumRows(); t += frames_per_eg) {
    ...
    Posterior labels_aux(frames_per_eg);
    for (int32 i = 0; i < actual_frames_per_eg; i++) {
      labels_aux[i] = aux_post[t + i];
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This will expect a single output_aux output for use for test amtl data.\n"
        ...
        "multi-task case where you have some input features, possibly some\n"
        "iVectors, and multiple (currently 2) sets of labels.\n"
        "\n"
        "Usage:  nnet3-get-egs-mt-aux-only [options] <features-rspecifier> "
        "<aux-task-rspecifier> <egs-out>\n"
        "\n"
        "An example [where $feats expands to the actual features]:\n"
        "nnet-get-egs-mt --num-pdfs=2658 --num_aux=8 --left-context=12 --right-context=9 --num-frames=8 \"$feats\"\\\n"
        "\"ark:gunzip -c exp/nnet/ali.1.gz | ali-to-pdf exp/nnet/1.nnet ark:- ark:- | ali-to-post ark:- ark:- |\" \\\n"
        " ark:aux_task.ark  ark:- \n";
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!aux_task_reader.HasKey(key)) {
        num_err++;
        KALDI_WARN << "No aux posterior for key " << key;
      } else {
        //const Posterior &pdf_post = pdf_post_reader.Value(key);
        const Posterior &aux_post = aux_task_reader.Value(key);
        if (aux_post.size() != feats.NumRows()) {
          KALDI_WARN << "Aux posterior has wrong size " << aux_post.size()
