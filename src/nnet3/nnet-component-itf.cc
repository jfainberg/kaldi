...
Component* Component::NewComponentOfType(const std::string &component_type) {
  ...
  } else if (component_type == "LogSoftmaxPenalisedComponent") {
    ans = new LogSoftmaxPenalisedComponent();
  ...
  } else if (component_type == "GradientScaleComponent") {
    ans = new GradientScaleComponent();
